...
COL_FILTER = COL_FILTER - 40

local COL_TOTAL = COL_STATE + 15
-------------------------
-- DIALOGS AND BUTTONS --
-------------------------
...
function FilterListDrawUpdate (self)
	...
	for count = 1, BUTTON_TOTAL do
		...
		if index <= #GlobalIgnoreDB.filterList then
			...
			FilterScrollButtons[count].total:SetText(GlobalIgnoreDB.filterCount[index])
...
local function CreateFilterButtons()
	...
	for count = 1, BUTTON_TOTAL do
		...
		FilterScrollButtons[count].total = FilterScrollButtons[count]:CreateFontString("FontString", "OVERLAY", "GameFontHighlight")
		FilterScrollButtons[count].total:SetPoint("LEFT", FilterScrollButtons[count].filter, "RIGHT", 6, 0)
		FilterScrollButtons[count].total:SetWidth(COL_TOTAL)
		FilterScrollButtons[count].total:SetJustifyH("LEFT")
		FilterScrollButtons[count].total:SetWordWrap(false)
	end
...
local function CreateUIFrames()
	...
	createColumn("Total", COL_TOTAL, Tab2Frame)
